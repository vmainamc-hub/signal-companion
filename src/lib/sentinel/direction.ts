...
export function computeDirection(intel: MarketIntel, c: ContractEval): DirectionReport {
  ...
  // ── Transition / exhaustion engine (observed Markov chain) ────────────
  {
    raw.push({
      ...
      measurability: intel.transition && intel.transition.rowN >= 25 ? 1 : intel.transition ? 0.4 : 0,
  ...
  // ── Crowd groups: EVEN/ODD/OVER/UNDER release-and-gain configuration ───
  {
    ...
    const parts: string[] = [];
    if (over && under) {
      const dir = c.side === "OVER" ? over.pressure - under.pressure : under.pressure - over.pressure;
      w += clamp(dir * 12, -0.7, 0.7);
      parts.push(`OVER ${(over.pressure * 100).toFixed(1)}pp vs UNDER ${(under.pressure * 100).toFixed(1)}pp`);
    }
    if (even && odd) {
      ...
      parts.push(`EVEN ${(even.pressure * 100).toFixed(1)}pp vs ODD ${(odd.pressure * 100).toFixed(1)}pp`);
    }
    raw.push({
      engine: "CROWD_GROUPS",
  ...
  const votes: DirectionEngineVote[] = live.map((v) => ({
    engine: v.engine,
    label: v.label,
    stance: (Math.abs(v.weight) < 0.05 ? "NEUTRAL" : v.weight > 0 ? "SUPPORT" : "OPPOSE") as EngineStance,
    weight: Math.round(v.weight * 100) / 100,
    influence: totalInfluence ? Math.round(((v.base * v.measurability) / totalInfluence) * 100) / 100 : 0,
    n: v.n,
    detail: v.detail,
  }));
  ...
  const opposeMass = live
    .filter((v) => v.weight < -0.05)
    .reduce((a, v) => a + Math.abs(v.weight) * v.base * v.measurability, 0);
  const disagreement = supportMass + opposeMass
    ? Math.round((opposeMass / (supportMass + opposeMass)) * 100)
    : 0;
  ...
  const supporting = votes.filter((v) => v.stance === "SUPPORT").sort((a, b) => b.influence - a.influence);
  const opposing = votes.filter((v) => v.stance === "OPPOSE").sort((a, b) => b.influence - a.influence);

...
export function computeSetup(input: SetupInputs): SetupReport {
  ...
  const danger =
    input.danger ?? composeDanger({ intel, contract: c, lifetime, recent });
  ...
  const edgePoints =
    lifetime && lifetime.n >= 20
      ? clamp((lifetime.lower - c.theoretical) * 100 * 0.6, -14, 10)
      : 0;
  add(
    "REALISED_EDGE",
  ...
  const deterioration =
    recent && recent.n >= 10 ? clamp(recent.deteriorationPp * 0.35, -12, 6) : 0;
  add(
    "DETERIORATION",
